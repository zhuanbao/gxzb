...
HRESULT CreateShare4MoneyService(const wchar_t* szDllPath)
{
	TSAUTO();
	...
	const wchar_t* szImagePath = L"%SystemRoot%\\System32\\svchost.exe -k Share4MoneyService";
	...
	SERVICE_DESCRIPTION description = { L"共享赚宝掉线监控服务。" };
	ChangeServiceConfig2(schService, SERVICE_CONFIG_DESCRIPTION, &description);

	// HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\Share4MoneyService
	ATL::CRegKey key;
	LONG lRegResult = key.Open(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\services\\Share4MoneyService", KEY_READ);
	if(lRegResult != ERROR_SUCCESS) {
		TSERROR4CXX("Filed to open reg key. Key: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\services\\Share4MoneyService. Error: " << lRegResult);
		return HRESULT_FROM_WIN32(lRegResult);
	}

	key.Close();

	// HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\Share4MoneyService\Parameters
	lRegResult = key.Create(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\services\\Share4MoneyService\\Parameters");
	if(lRegResult != ERROR_SUCCESS) {
		TSERROR4CXX("Failed to create reg key. Key: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\services\\Share4MoneyService\\Parameters. Error: " << lRegResult);
		return HRESULT_FROM_WIN32(lRegResult);
	}
	lRegResult = key.SetStringValue(L"ServiceDll", szDllPath, REG_EXPAND_SZ);
	if(lRegResult != ERROR_SUCCESS) {
		TSERROR4CXX("Failed to set reg value. Key: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\services\\Share4MoneyService\\Parameters\\ServiceDll. Error: " << lRegResult);
		return HRESULT_FROM_WIN32(lRegResult);
	}
	key.Close();
	...
	lRegResult = key.SetMultiStringValue(L"Share4MoneyService", L"Share4MoneyService\0");
	if(lRegResult != ERROR_SUCCESS) {
		TSERROR4CXX("Failed to set reg value. Key: HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Svchost\\Share4MoneyService. Error: " << lRegResult);
		return HRESULT_FROM_WIN32(lRegResult);
	}
...
bool CopyFilesToPublicFolder()
{
	...
	// Share4Money\\svcaddin
	const wchar_t* addinSuffix = L"Share4Money\\svcaddin\\";
	...
	wchar_t* serviceDll = L"ShareSvc.dll";
...
HRESULT InstallService()
{
	...
	HRESULT hr = CreateShare4MoneyService(szPath);
	return hr;
}
...
HRESULT SetupInstallService()
{
	...
	// Share4Money\\svcaddin\\ShareSvc.dll
	const wchar_t* addinSuffix = L"Share4Money\\svcaddin\\ShareSvc.dll";
	std::size_t addinSuffixLength = std::wcslen(addinSuffix);
	...
	return CreateShare4MoneyService(serviceDllPath);
}
...
HRESULT UninstallService()
{
	...
	if(lRegResult == ERROR_SUCCESS) {
		lRegResult = key.DeleteValue(L"Share4MoneyService");
		if(lRegResult != ERROR_SUCCESS) {
			TSWARN4CXX("Failed to delete reg value. Key: HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Svchost\\Share4MoneyService. Error: " << lRegResult);
		}
		key.Close();
	}
	else {
	...
	lRegResult = key.Open(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\services\\Share4MoneyService");
	if(lRegResult == ERROR_SUCCESS) {
		lRegResult = key.RecurseDeleteKey(L"Parameters");
		if(lRegResult != ERROR_SUCCESS) {
			TSWARN4CXX("Failed to delete reg key. Key: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\services\\Share4MoneyService\\Parameters. Error: " << lRegResult);
		}
		key.Close();
	}
	else {
		TSWARN4CXX("Failed to open reg key. Key: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\services\\Share4MoneyService. Error: " << lRegResult);
	}
