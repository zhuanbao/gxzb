...
#include "../Utility/InjectHelper.h"
#include <comdef.h>
...
XLLRTGlobalAPI LuaAPIUtil::sm_LuaMemberFunctions[] = 
{
	...
	{"InjectDllToProc", InjectDllToProc},
...
int LuaAPIUtil::CloseLuaMsgWnd(lua_State* pLuaState)
{
	LuaMsgWindow::Instance()->CloseWnd();
	return 0;
}
...
int LuaAPIUtil::GetProcessSnapshot(lua_State* pLuaState)
{
	EnablePrivilegeHelper(::GetCurrentProcess(), SE_DEBUG_NAME, TRUE);
	...
	if (INVALID_HANDLE_VALUE != hProcessSnapshot)
	{
		...
		for (BOOL bEnumResult = ::Process32FirstW(hProcessSnapshot, &pe); 
			bEnumResult; bEnumResult = ::Process32NextW(hProcessSnapshot, &pe))
		{
			if (pe.th32ProcessID >= 0)
			{
				...
				// process id
				{
					lua_pushstring(pLuaState, "PID");
					lua_pushinteger(pLuaState, pe.th32ProcessID);
					lua_settable(pLuaState, -3);
				}
				// process parent id
				if (pe.th32ParentProcessID > 0)
				{
					lua_pushstring(pLuaState, "PPID");
					lua_pushinteger(pLuaState, pe.th32ParentProcessID);
					lua_settable(pLuaState, -3);
				}
				if (pe.szExeFile)
				{
					std::string strExeName = ultra::_T2UTF(pe.szExeFile);
					lua_pushstring(pLuaState, "ExeName");
					lua_pushstring(pLuaState, strExeName.c_str());
					lua_settable(pLuaState, -3);
				}
				// process handle
				...
				if (hProcess)
				{
					lua_pushstring(pLuaState, "Handle");
					lua_pushlightuserdata(pLuaState, hProcess);
					lua_settable(pLuaState, -3);
				}
				// process lower handle
				HANDLE hLowProcess = ::OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_READ|SYNCHRONIZE, FALSE, pe.th32ProcessID);
				if (hLowProcess)
				{
					lua_pushstring(pLuaState, "LowHandle");
					lua_pushlightuserdata(pLuaState, hLowProcess);
					lua_settable(pLuaState, -3);
				}
				else
				{
					DWORD dwError = GetLastError();
					TSDEBUG(L"get process[%d] low handle error=%d", pe.th32ProcessID, dwError);
				}

				// process image path
				if (hLowProcess)
				{
					WCHAR wszImagePath[MAX_PATH + 1] = {0};
					::GetModuleFileNameEx(hProcess, NULL, wszImagePath, MAX_PATH);
					WCHAR wszImageFullPath[MAX_PATH + 1] = {0};
					::ExpandEnvironmentStringsW(wszImagePath, wszImageFullPath, MAX_PATH);
					std::string strImagePath = ultra::_T2UTF(wszImagePath);
					lua_pushstring(pLuaState, "ImagePath");
					lua_pushstring(pLuaState, strImagePath.c_str());
					lua_settable(pLuaState, -3);
				}
				lua_rawseti(pLuaState, -2, ++nProcessNumbers);
			}
		}
		return 1;
	}
...
int LuaAPIUtil::InjectDllToProc(lua_State* pLuaState)
{
	DWORD dwProcessId = static_cast< DWORD >(lua_tointeger(pLuaState, 2));
	const char* filePath = luaL_checkstring(pLuaState, 3);
	std::wstring wstrDllPath = ultra::_UTF2T(filePath);
	bool bRet = InjectDll(dwProcessId,wstrDllPath.c_str());
	lua_pushboolean(pLuaState, bRet);
	return 1;
}

int LuaAPIUtil::RestartSystem(lua_State* pLuaState)
{
	DWORD dwTimeout = static_cast< DWORD >(lua_tointeger(pLuaState, 2));
	BOOL bForceAppsClosed = lua_toboolean(pLuaState, 3);
	BOOL bRebootAfterShutdown  = lua_toboolean(pLuaState, 4);
	if (EnablePrivilegeHelper(::GetCurrentProcess(), SE_SHUTDOWN_NAME, TRUE))
	{
		InitiateSystemShutdown(NULL,NULL,dwTimeout,bForceAppsClosed,bRebootAfterShutdown);
	}
	return 0;
}