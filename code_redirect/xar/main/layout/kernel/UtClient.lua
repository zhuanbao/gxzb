...
local g_PoolCfgName = "dpcfg.json"
local g_DefaultPoolType = "x_cd"
local g_nPlatformId = 0
--常量
local CLIENT_UTC = 8
local CLIENT_PATH = "Share4Peer\\Share4PeerUC.exe"
...
local WP_UT_SHARE = 2
--连接矿次结果
local WP_UT_CONNECT_POOL = 3
--错误
--进程自动退出
local WP_UT_AUTOEXIT = 5
--解析到错误信息
local WP_UT_ERROR_INFO = 9
...
local g_CpuUsage_Min = 50
local g_CpuUsage_Max = 75
local g_CpuUsage_Cur = g_CpuUsage_Min
...
local g_UtDAGTimerId = nil

local g_UtRealTimeIncomeTimerId = nil
...
function TipLog(strLog)
	if type(tipUtil.Log) == "function" then
		tipUtil:Log("UtClient: " .. tostring(strLog))
	end
end
...
function GetNewMiningCmdInfo()
	...
	if g_PoolIndex <= #tPoolList then
		...
		if type(tabPoolItem) == "table" then
			if IsRealString(tabPoolItem["cmdlineformat_c"]) then
				g_strCmdLineFormat = tabPoolItem["cmdlineformat_c"]
...
-- -o "stratum+tcp://121.196.200.110:7100" -u "UgaeXvZEHSdqf3J1zKcmdVDv41jW1jcbMm.tt" -p "" -t 2 --max-cpu-usage 75
...
function GenerateVirtualDAG()
	if g_UtDAGTimerId == nil then
		TipLog("[GenerateVirtualDAG] start virtual dag")
		local nProgress = 1
		g_UtDAGTimerId = timeMgr:SetTimer(function(Itm, id)
			if nProgress >= 100 then
				timeMgr:KillTimer(g_UtDAGTimerId)
				g_UtDAGTimerId = nil
				return 
			end
...
function StartRealTimeIncomeTimer()
	if g_UtRealTimeIncomeTimerId ~= nil then
		return
	end
	g_UtRealTimeIncomeTimerId = timeMgr:SetTimer(function(Itm, id)
		local nSpanTime = 0
...
function OnUtMsg(tParam)
	local nMsgType, nParam = tParam[1],tParam[2]
	TipLog("[OnUtMsg] nMsgType = " .. GTV(nMsgType) .. ", nParam = " .. GTV(nParam))
	if nMsgType == WP_UT_SPEED then
		SupportClientType:SetNoCrashFlag(CLIENT_UTC)
		KillVirtualDAG()
	...
	elseif nMsgType == WP_UT_SHARE then
		--KillVirtualDAG()
		g_LastClientOutputRightInfoTime = tipUtil:GetCurrentUTCTime()
		g_PreWorkState = CLIENT_STATE_CALCULATE
		if nParam == 0 then
	...
	elseif nMsgType == WP_UT_CONNECT_POOL then
		if nParam == 0 then
			g_LastClientOutputRightInfoTime = tipUtil:GetCurrentUTCTime()
			g_ConnectFailCnt = 0
			if g_PreWorkState ~= CLIENT_STATE_CALCULATE then
	...
	elseif nMsgType == WP_UT_AUTOEXIT then
		...
		ReTryStartClient()
	elseif nMsgType == WP_UT_ERROR_INFO then
		KillVirtualDAG()
...
function InitCmdLine()
	...
	if nWorkModel == 1 then
		g_CpuUsage_Cur = g_CpuUsage_Min
		SetControlSpeedCmdLine(g_CpuUsage_Min)
	else
		g_CpuUsage_Cur = g_CpuUsage_Max
		SetControlSpeedCmdLine(g_CpuUsage_Max)
	end
end

function SetControlSpeedCmdLine(nCpuUsage)
	if nCpuUsage ~= nil then
		g_ControlSpeedCmdLine = "--max-cpu-usage " .. tostring(nCpuUsage)
	else
...
function ChangeMiningSpeed()
	local nWorkModel = UIInterface:GetCurrentWorkModel()
	if nWorkModel ~= 1 then
		if g_CpuUsage_Cur ~= g_CpuUsage_Max then
			g_CpuUsage_Cur = g_CpuUsage_Max
			SetControlSpeedCmdLine(g_CpuUsage_Max)
			Quit()
			Start()			
		...
		if LimitSpeedCond() then
			if g_CpuUsage_Cur ~= g_CpuUsage_Min then
				g_CpuUsage_Cur = g_CpuUsage_Min
				SetControlSpeedCmdLine(g_CpuUsage_Min)
				Quit()
				Start()	
...
--3分钟还没有纠错 就当已经不能挖矿了
function StartUtTimer()
	if g_UtWorkingTimerId then
		timeMgr:KillTimer(g_UtWorkingTimerId)
		g_UtWorkingTimerId = nil
	end
	g_LastClientOutputRightInfoTime = tipUtil:GetCurrentUTCTime()
	g_UtWorkingTimerId = timeMgr:SetTimer(function(Itm, id)
		...
		if g_PreWorkState == CLIENT_STATE_EEEOR and  nCurrentTime - g_LastClientOutputRightInfoTime > 60 then
			TipLog("[StartUtTimer] error occur and correct time out, try to restart")
			ReTryStartClient()
		elseif nCurrentTime - g_LastClientOutputRightInfoTime > 3*60 then
			g_PreWorkState = CLIENT_STATE_TIMEOUT
			TipLog("[StartUtTimer] output time out, try to restart")
			ReTryStartClient()
		end
...
function ResetGlobalParam()
	...
	if g_UtRealTimeIncomeTimerId then
		timeMgr:KillTimer(g_UtRealTimeIncomeTimerId)
		g_UtRealTimeIncomeTimerId = nil
	end
	...
	SupportClientType:ClearCrashDebugFlag(CLIENT_UTC)
end
...
function InitClient(nPlatformId, nMiningType)
	...
	CLIENT_UTC = nMiningType
	IPCUtil:Init(CLIENT_UTC)
end
function Start()
	...
	SupportClientType:SetCrashDebugFlag(CLIENT_UTC)
...
function ReStartClientByNextPool()
	TipLog("[ReStartClientByNextPool] restart")
	Quit()
	TipLog("[ReStartClientByNextPool] quit finish")
...
function ReTryStartClient()
	...
	TipLog("[ReTryStartClient] start current client")
	if Start() ~= 0 then
		TipLog("[ReTryStartClient] start error")
		UIInterface:SetStateInfoToUser("获取赚宝任务失败,请稍后再试")
		ClientWorkModule:QuitMinerSuccess()
		TipLog("[ReTryStartClient] start error2")
		return
	end
end
