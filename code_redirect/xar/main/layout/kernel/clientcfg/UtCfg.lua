...
function TipLog(strLog)
	if type(tipUtil.Log) == "function" then
		tipUtil:Log("UtCfg: " .. tostring(strLog))
	end
end
	...
	if nPoolIndex <= #tPoolList then
		...
		if type(tabPoolItem) == "table" then
			if IsRealString(tabPoolItem["cmdlineformat_c"]) then
				strPoolParam = tabPoolItem["cmdlineformat_c"]
...
function InitClient(nPlatformId)
	...
	g_tabClientInfo["tabParam"] = {
		["strCoinType"] = "cd",
		["strPoolVerKey"] = "p",
		["strDefaultPoolType"] = "x_cd",
		["strPoolFileName"] = "dpcfg.json",
		["strClientPath"] = "Share4Peer\\Share4PeerUC.exe",
	...
	g_tabClientInfo["strMinSpeedParam"] = "--max-cpu-usage 50"
	g_tabClientInfo["strMaxSpeedParam"] = "--max-cpu-usage 75"
	g_tabClientInfo["strPoolParam"] = nil
