...
function TipLog(strLog)
	if type(tipUtil.Log) == "function" then
		tipUtil:Log("ZcaCfg: " .. tostring(strLog))
	end
end
	...
	if nPoolIndex <= #tPoolList then
		...
		if type(tabPoolItem) == "table" then
			if IsRealString(tabPoolItem["cmdlineformat_a"]) then
				strPoolParam = tabPoolItem["cmdlineformat_a"]
...
function InitClient(nPlatformId)
	...
	g_tabClientInfo["tabParam"] = {
		...
		["strClientPath"] = "Share4Peer\\Share4PeerZA.exe",
		["nMaxConnectFailCnt"] = 7,
		["nSpeedMultiplier"] = 1000,
	}
	...
	g_tabClientInfo["strMinSpeedParam"] = "-i 0"
...
function RegisterFunctionObject(self)
	...
	obj.ResetPoolIndex = ResetPoolIndex
