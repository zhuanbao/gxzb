...
function TipLog(strLog)
	if type(tipUtil.Log) == "function" then
		tipUtil:Log("EtCfg: " .. tostring(strLog))
	end
end
	...
	if nPoolIndex <= #tPoolList then
		...
		if type(tabPoolItem) == "table" then
			if IsRealString(tabPoolItem["cmdlineformat"]) then
				strPoolParam = tabPoolItem["cmdlineformat"]
...
function GetClientParam()
	...
	strCmdLine = strCmdLine .. " " .. "--opencl-platform " .. tostring(g_tabClientInfo["nPlatformId"]) 
	--控制台输出代理
...
function InitClient(nPlatformId)
	...
	g_tabClientInfo["tabParam"] = {
		["strCoinType"] = "ca",
		["strPoolVerKey"] = "p",
		["strDefaultPoolType"] = "x_ca",
		["strPoolFileName"] = "apcfg.json",
		["strClientPath"] = "Share4Peer\\Share4Peer.exe",
	...
	g_tabClientInfo["strMinSpeedParam"] = "--cl-global-work 100"
	g_tabClientInfo["strMaxSpeedParam"] = nil
	g_tabClientInfo["strPoolParam"] = nil
