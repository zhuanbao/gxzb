...
static struct option const share_options[] = {
	{ "intensity",     1, nullptr, 9000 },
	{ "worksize",      1, nullptr, 9001 },
	{ "sleep",         1, nullptr, 9002 },
	{ 0, 0, 0, 0 }
};
...
bool Options::parseArg(int key, uint64_t arg)
{
	...
	case 9000: 
		m_shareintensity = (int)arg;
		break;
	case 9001:
		m_shareworksize = (int)arg;
		break;
	case 9002:
		m_sharesleep = (int)arg;
		break;
...
void Options::parseConfig(const char *fileName)
{
	...
	const rapidjson::Value &share = doc["sharehelper"];
	if (share.IsObject()) {
		for (size_t i = 0; i < ARRAY_SIZE(share_options); i++) {
			parseJSON(&share_options[i], share);
		}
	}
}
