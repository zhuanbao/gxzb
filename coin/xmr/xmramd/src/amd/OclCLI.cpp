...
void OclCLI::autoConf(std::vector<OclThread*> &threads, int *platformIndex)
{
    *platformIndex = getAMDPlatformIdx();
    ...
    for (GpuContext &ctx : devices) {
		...
		int Intensity = std::min(maxThreads, maxIntensity);
		const size_t possibleIntensity = std::min(Intensity, Options::i()->shareIntensity());
        const size_t intensity         = (possibleIntensity / (8 * ctx.computeUnits)) * ctx.computeUnits * 8;

		const size_t worksize          = std::min(8, Options::i()->shareWorksize());
        threads.push_back(new OclThread(ctx.deviceIdx, intensity, worksize));
    }
}
